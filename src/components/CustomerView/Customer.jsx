...
function Customer() {
  ...
  <div className="text-lg font-semibold mb-2 flex items-center gap-2 text-white">
    <svg width="22" height="22" fill="none" className="inline" viewBox="0 0 24 24">
      <path fill="#9CA3AF" d="M12 12c2.7 0 8 1.34 8 4v2a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-2c0-2.66 5.3-4 8-4Zm0-2a4 4 0 1 1 0-8 4 4 0 0 1 0 8Z"/>
    </svg>
    Your Profile
  </div>
  ...
  <div className="mb-2 flex items-center gap-2">
    ...
    <span className="font-bold text-green-400">{customer.creditScore}</span>
    <span className="text-xs bg-green-200 text-green-700 px-2 py-0.5 rounded-full ml-2">{customer.creditScoreStatus}</span>
  </div>
  <div className="mb-2">
    <span className="text-gray-300">Annual Income:</span>
    <span className="font-bold ml-2 text-white">${customer.annualIncome.toLocaleString()}</span>
  </div>
  <div className="mb-4">
  ...
  <div>
    ...
    <ul>
      {customer.spendingInsights.map((item) => (
        <li key={item.label} className="flex justify-between mb-1 text-[15px]">
          <span className="text-gray-300">{item.label}</span>
          <span className="font-medium text-gray-100">${item.value.toLocaleString()}/month</span>
        </li>
      ))}
    </ul>
  </div>
...
<div className="bg-[#1D2041] border border-gray-700 shadow-lg rounded-xl p-6 mb-4 h-[620px] flex flex-col">
  <div className="text-xl font-semibold mb-1 text-white">Top Recommendations for You</div>
  <div className="text-sm text-gray-400 mb-4">Based on your spending habits and financial profile</div>{/* Card 
